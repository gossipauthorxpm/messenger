...
public class OnlineSocketThread extends Thread {
    ...
    public void run() {
        while (this.running) {
            try {
                sleep(1000);
                log.info("The data of the user {} friends is transferred to the web socket topic", this.user.getId());
                List<User> friendsUser = this.friendRequestService.getAllFriendsForUser(this.user);
                this.messagingTemplate.convertAndSend(String.format("/topic/friends/%s", this.user.getId()), friendsUser);
